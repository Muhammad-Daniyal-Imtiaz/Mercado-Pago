...
export function AdminSidebar() {
  ...
  const navItems = [
    { name: 'Mi Equipo de trabajo', href: '/dashboard/account_admin' },
  ...
  return (
    <aside className="w-64 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 transition-colors min-h-[calc(100vh-64px)]">
      <nav className="p-4 space-y-2">
        <div className="px-4 py-2 mb-4 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Panel del Administrador</div>
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
