# Guía de Experimentación del Sistema de Roles

## Objetivo
Probar el sistema de sincronización automática de roles y verificar que las garantías funcionan correctamente.
...
4. Busca tu email y edita el campo `roles` a:
```json
...
2. **Regístrate como nuevo usuario** (o usa tu cuenta existente)
...
WHERE email = 'guillermoandrada@gmail.com';
```
...
curl http://localhost:3001/api/admin/health/roles
```
...
curl -X POST http://localhost:3001/api/auth/sync-roles \
...
4. Verifica configuración de role-guards
...
Esta guía te permite experimentar completamente con el sistema y verificar que todas las garantías funcionen correctamente.
